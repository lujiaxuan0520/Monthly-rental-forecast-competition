...
if __name__=='__main__':
    ...
    # 读取特征选择后的特征
    filter_feature = pd.read_pickle('filter_feature_top250.pickle')
    ...
    for i in range(epoch):
        ...
        ada = AdaBoostRegressor(ada_tree_backing, n_estimators=600, learning_rate=0.001, loss='square')
        ada,predict_y_rf=regression_model(ada,train_data, train_y, test_data, filter_feature)
    ...
    for i in range(56283):
        avg[i]=res[i].mean()
    #将预测结果导出
    np.savetxt('avg_predict_result_ada.csv',avg,delimiter=',')
    np.savetxt('all_predict_result_ada.csv',res,delimiter=',')
